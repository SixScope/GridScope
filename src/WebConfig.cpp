...
String WebConfig::buildHtml() {
    ...
    html += "    <span id='minPwmVal' style='color:#f39c12;font-weight:bold;'>" + String(currentConfig.minPwm) + "</span>";
    html += "  </div>";
    html += "  <input type='range' name='minPwm' min='0' max='255' value='" + String(currentConfig.minPwm) + "' class='slider' oninput='document.getElementById(\"minPwmVal\").innerText=this.value; updateBrightnessLive(\"minPwm\", this.value);'>";
    html += "</div>";
    
    html += "<div style='margin-bottom:15px;'>";
    html += "  <div style='display:flex;justify-content:space-between;margin-bottom:5px;'>";
    html += "    <label>Daylight Threshold LDR (1-100%)</label>";
    html += "    <span id='maxLdrVal' style='color:#f39c12;font-weight:bold;'>" + String(currentConfig.maxLdrPct) + "%</span>";
    html += "  </div>";
    html += "  <input type='range' name='maxLdr' min='1' max='100' value='" + String(currentConfig.maxLdrPct) + "' class='slider' oninput='document.getElementById(\"maxLdrVal\").innerText=this.value+\"%\"; updateBrightnessLive(\"maxLdr\", this.value);'>";
    html += "</div>";
